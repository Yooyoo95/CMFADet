...
from .ops_dcnv3.modules import DCNv3 ####2025-08#####
...
__all__ = ['Detect_DyHead', 'Detect_DyHeadWithDCNV3', 'Detect_DyHeadWithDCNV4', 'Detect_AFPN_P345', 'Detect_AFPN_P345_Custom', 'Detect_AFPN_P2345', 'Detect_AFPN_P2345_Custom', 'Detect_Efficient', 'DetectAux',
           'Segment_Efficient', 'Detect_SEAM', 'Detect_MultiSEAM', 'Detect_DyHead_Prune', 'Detect_LSCD', 'Segment_LSCD', 'Pose_LSCD', 'OBB_LSCD', 'Detect_ATAH', 'Segment_ATAH', 'Pose_ATAH', 'OBB_ATAH','OBB_ATAH_v2',
...
class TaskawareFeatureModulator(nn.Module):
    ...
    def forward(self, feat, avg_feat=None):
        ...
        # avg_feat_enriched = torch.cat([pool_1x1, pool_1x1_max], dim=1)
        # pool_2x2 = F.adaptive_avg_pool2d(feat, (2, 2))
        # pool_2x2 = F.interpolate(pool_2x2, size=(1, 1), mode='bilinear', align_corners=False)

        if avg_feat is None:
            # avg_feat = F.adaptive_avg_pool2d(feat, (1, 1))
            avg_feat_enriched = pool_1x1 + pool_1x1_max # 2025-08
            # avg_feat_enriched = pool_1x1 + pool_1x1_max + pool_2x2 # 2025-08
        else:
            avg_feat_enriched = avg_feat # 2025-07-22
            avg_feat_enriched = pool_1x1 + pool_1x1_max
            # avg_feat_enriched = pool_1x1 + pool_1x1_max + pool_2x2

        # weight = self.relu(self.la_conv1(avg_feat))
        weight = self.relu(self.la_conv1(avg_feat_enriched)) # 2025-08
...
class Detect_ATAH(nn.Module):
    # Adaptive Task-aware Alignment Head
    """YOLOv8 Detect head for detection models."""
    ...
    def __init__(self, nc=80, hidc=256, ch=()):
        ...
        # self.DyDCNV2 = DCNv2(hidc // 2, hidc // 2)  ####2025-08####
...
class Segment_ATAH(Detect_ATAH):
    """YOLOv8 Segment head for segmentation models."""
    ...
    def __init__(self, nc=80, nm=32, npr=256, hidc=256, ch=()):
        ...
        self.npr = npr  # number of protos
        self.proto = Proto(ch[0], self.npr, self.nm)  # protos
        self.detect = Detect_ATAH.forward
...
######################ATAH_v2 2025-08##################
# --- OBB_ATAH_v2
class OBB_ATAH_v2(nn.Module):
    def __init__(self, nc, ne, hidc, ch):
        super().__init__()
        self.nc = nc
        self.ne = ne
        self.hidc = hidc
        ...
        self.share_conv = Conv_GN_WS(self.ch[0], self.hidc // 2, 3)

        self.cls_branch = TaskawareFeatureModulator(self.hidc // 2, 2, 16)
        self.reg_branch = TaskawareFeatureModulator(self.hidc // 2, 2, 16)

        self.avg_feat_channel_adapter = nn.Conv2d(self.hidc // 2, self.hidc, kernel_size=1)


    def forward(self, x):

        feat = self.share_conv(x[0])
        avg_feat = F.adaptive_avg_pool2d(feat, (1, 1))

        adapted_avg_feat = self.avg_feat_channel_adapter(avg_feat)

        cls_feat = self.cls_branch(feat, adapted_avg_feat)
        reg_feat = self.reg_branch(feat, adapted_avg_feat)
