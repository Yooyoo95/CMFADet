...
class BaseDataset(Dataset):
    ...
    def __init__(
        self,
        img_path,
        imgir_path, ### 2025-08 ### Infrare images
        imgsz=640,
        cache=False,
        augment=True,
        hyp=DEFAULT_CFG,
        ...
        self.imgir_path = imgir_path ### 2025-08 ###
        self.imgsz = imgsz
        ...
        self.imir_files = self.get_img_files(self.imgir_path) ### 2025-08 ###
        self.labels = self.get_labels()
        # self.labelsir = self.get_labels() ### 2025-08 ###
        ...
        # self.npyir_files = [Path(f).with_suffix(".npy") for f in self.imir_files] ## 2025-08
        if cache:
            self.cache_images(cache)
            # self.cache_images_ir() ## 2025-08
    ...
    def cache_images_ir(self, cache):
        ...
        fcn, storage = (self.cache_images_ir_to_disk, "Disk") if self.cache == "disk" else (self.load_image_ir, "RAM")
        with ThreadPool(NUM_THREADS) as pool:
            results = pool.imap(fcn, range(self.ni))
            pbar = TQDM(enumerate(results), total=self.ni, disable=LOCAL_RANK > 0)
            for i, x in pbar:
    ...
    def cache_images_ir_to_disk(self, i):
        """Saves an image as an *.npy file for faster loading."""
    ...
    def set_rectangle(self):
        ...
        self.imir_files = [self.imir_files[i] for i in irect] ##2025-08
